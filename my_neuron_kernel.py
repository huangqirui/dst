...
from spikingjelly.clock_driven import cu_kernel_opt, surrogate, tensor_cache
...
class MultiStepLIFNodePTT(torch.autograd.Function):
    ...
    def create_fptt_kernel(decay_input: bool, hard_reset: bool, dtype: str, kernel_name_prefix: str = 'LIFNode'):
        ...
        if dtype == 'fp32':
            ...
            const float & v_threshold, {'const float & v_reset,' if hard_reset else ''}
            const int & neuron_num, const int & numel, const float* mask_seq)
            '''
            code += r'''
            {
            const int index = blockIdx.x * blockDim.x + threadIdx.x;
            ...
            code += r'''
                    ...
                    v_v_seq[t + dt] = v_v_seq[t + dt] * mask_seq[t];
            ...
            const half & v_threshold, {'const half & v_reset,' if hard_reset else ''}
            const int & neuron_num, const int & numel, const half2* mask_seq) 
            '''
            ...
            code += r'''
                    v_v_seq[t + stride] = __hmul2(v_v_seq[t + stride], mask_seq[t]);
    ...
    def forward(ctx, x_seq: torch.Tensor, v_last: torch.Tensor, decay_input: bool, tau: float, v_threshold: float,
                v_reset: float, detach_reset: bool, sg_cuda_code_fun, mask_seq: torch.Tensor):
        requires_grad = x_seq.requires_grad or v_last.requires_grad
        device = x_seq.get_device()
        if x_seq.dtype == torch.float32:
            dtype = 'fp32'
        ...
        with cu_kernel_opt.DeviceEnvironment(device):
            numel = x_seq.numel()   # T*N
            neuron_num = numel // x_seq.shape[0]    # N
            ...
            if v_reset is None:
                ...
                cp_v_reset = cupy.asarray(v_reset, dtype=cp_dtype)
                hard_reset = True
                x_seq, v_v_seq, h_seq, spike_seq, cp_reciprocal_tau, cp_v_threshold, cp_v_reset, cp_neuron_num, cp_numel, mask_seq = cu_kernel_opt.get_contiguous(
                ...
                kernel_args = [x_seq, v_v_seq, h_seq, spike_seq, cp_reciprocal_tau, cp_v_threshold, cp_v_reset,
                               cp_neuron_num, cp_numel, mask_seq]
...
class MultiStepParametricLIFNodePTT(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, x_seq: torch.Tensor, v_last: torch.Tensor, reciprocal_tau: torch.Tensor, decay_input: bool,
                v_threshold: float,
...
class MultiStepEIFNodePTT(torch.autograd.Function):
    ...
    def forward(ctx, x_seq: torch.Tensor, v_last: torch.Tensor, tau: float, v_threshold: float, v_reset: float,
        ...
        with cu_kernel_opt.DeviceEnvironment(device):
            ...
            if v_reset is None:
                cp_v_reset = None
                hard_reset = False
                x_seq, v_v_seq, h_seq, spike_seq, cp_reciprocal_tau, cp_delta_T, cp_theta_rh, cp_v_threshold, cp_v_rest, cp_neuron_num, cp_numel = cu_kernel_opt.get_contiguous(
                ...
                cp_v_reset = cupy.asarray(v_reset, dtype=cp_dtype)
                hard_reset = True
                x_seq, v_v_seq, h_seq, spike_seq, cp_reciprocal_tau, cp_delta_T, cp_theta_rh, cp_v_threshold, cp_v_rest, cp_v_reset, cp_neuron_num, cp_numel = cu_kernel_opt.get_contiguous(
                    x_seq, v_v_seq, h_seq, spike_seq, cp_reciprocal_tau, cp_delta_T, cp_theta_rh, cp_v_threshold,
    ...
    def backward(ctx, grad_spike_seq, grad_v_seq):
        ...
        with cu_kernel_opt.DeviceEnvironment(device):
            ...
            if hard_reset:
                grad_spike_seq, grad_v_seq, h_seq, spike_seq, v_v_seq, grad_x_seq, grad_v_last, ctx.cp_theta_rh, ctx.cp_reciprocal_delta_T, ctx.cp_reciprocal_tau, ctx.cp_one_sub_reciprocal_tau, ctx.cp_v_threshold, ctx.cp_v_reset, ctx.cp_neuron_num, ctx.cp_numel = cu_kernel_opt.get_contiguous(
                ...
                grad_spike_seq, grad_v_seq, h_seq, spike_seq, v_v_seq, grad_x_seq, grad_v_last, ctx.cp_theta_rh, ctx.cp_reciprocal_delta_T, ctx.cp_reciprocal_tau, ctx.cp_one_sub_reciprocal_tau, ctx.cp_v_threshold, ctx.cp_neuron_num, ctx.cp_numel = cu_kernel_opt.get_contiguous(
                    grad_spike_seq, grad_v_seq, h_seq, spike_seq, v_v_seq, grad_x_seq, grad_v_last, ctx.cp_theta_rh,
                    ctx.cp_reciprocal_delta_T, ctx.cp_reciprocal_tau, ctx.cp_one_sub_reciprocal_tau, ctx.cp_v_threshold,
                    ctx.cp_neuron_num, ctx.cp_numel)
                kernel_args = [grad_spike_seq, grad_v_seq, h_seq, spike_seq, v_v_seq, grad_x_seq, grad_v_last,
                        ...
                        if ms_neu == MultiStepLIFNodePTT or ms_neu == MultiStepParametricLIFNodePTT:
                            for decay_input in [True, False]:
                                ...
                                for detach_reset in [True, False]:
                                    ...
                                    bp_codes = ms_neu.create_bptt_kernel(sg().cuda_code, decay_input, hard_reset,
                                                                         detach_reset,
                                                                         dtype).code
