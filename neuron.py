...
def drop_path(x, drop_prob: float = 0., training: bool = False):
    ...
    shape = x.shape  # work with diff dim tensors, not just 2D ConvNets
...
class MultiStepLIFNode(LIFNode):
    def __init__(self, tau: float = 2., decay_input: bool = True, v_threshold: float = 1.,
                 ...
                 pa=0.0, pr=0.0, masking_type='random'):
    ...
    def forward(self, x_seq: torch.Tensor, cmp: torch.Tensor = None):
        assert x_seq.dim() > 1
        ...
        if self.backend == 'torch':
            ...
            if (self.pr == 0.0 and self.pa == 0.0) or not self.training:
                ...
                if self.pr != 0.0:
                    keep_prob = 1 - self.pr
                    mask_cur = self.mask_generator_x(keep_prob, x_seq.shape, dtype=x_seq.dtype, device=x_seq.device)
                    x_seq = x_seq * mask_cur
                    x_seq = x_seq * (torch.prod(torch.tensor(mask_cur.size())) / torch.sum(mask_cur))

                if self.pa != 0.0:
                    keep_prob = 1 - self.pa
                    mask = self.mask_generator_mp(keep_prob, x_seq.shape, dtype=x_seq.dtype, device=x_seq.device)
                    spike_seq, self.v_seq = my_neuron_kernel.MultiStepLIFNodePTT.apply(
                        x_seq.flatten(1), self.v.flatten(0),
                        self.decay_input, self.tau, self.v_threshold, self.v_reset, self.detach_reset,
                        self.surrogate_function.cuda_code,
                        mask)
                else:
                    spike_seq, self.v_seq = neuron_kernel.MultiStepLIFNodePTT.apply(
    ...
    def mask_generator_x(self, keep_ratio, shape, dtype, device):
        T, B, D, N = shape
        t = T - 1

        noise = None
        if self.masking_type == 'random':
            noise = torch.rand((t, B, N), dtype=dtype, device=device)
        elif self.masking_type == 'tube':
            noise = torch.rand((B, N), dtype=dtype, device=device)
            noise = noise.unsqueeze(0).repeat(t, 1, 1)
        elif self.masking_type == 'timestep':
            noise = torch.rand((t, B), dtype=dtype, device=device)
            noise = noise.unsqueeze(-1).repeat(1, 1, N)

        mask = keep_ratio + noise
        mask.floor_()

        mask = torch.cat((torch.ones((B, N), dtype=dtype, device=device).unsqueeze(0), mask))
        mask = mask.unsqueeze(-2).repeat(1, 1, D, 1)

        return mask

    def mask_generator_mp(self, keep_prob, shape, dtype, device):
        noise = None
        if self.masking_type == 'random':
            noise = torch.rand(shape, dtype=dtype, device=device)
        elif self.masking_type == 'tube':
            noise = torch.rand(shape[1:], dtype=dtype, device=device)
            noise = noise.unsqueeze(0).repeat(shape[0], 1, 1, 1)
        elif self.masking_type == 'timestep':
            noise = torch.rand(shape[:2], dtype=dtype, device=device)
            noise = noise.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, shape[2], shape[3])
        mask = keep_prob + noise
        mask.floor_()
        mask = mask.flatten(1)
        return mask
